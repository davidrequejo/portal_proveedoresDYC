...
$("#guardar_registro_area_p").on("click", function (e) { $("#submit-form-area_p").submit(); });   


function show_hide_escenario(flag) {
  if (flag == 1) {            // Tabla principal
    $('#div-tabla-principal-area_p').show();
    $(".btn-agregar-area_p").show();
    $(".btn-cancelar").hide();
    
  } else if (flag == 2) {     // Detalle proyecto
    $('#div-tabla-principal-area_p').hide();
    $(".btn-agregar-area_p").hide();
    $(".btn-cancelar").show();
  } else if (flag == 3) {     //
  } else if (flag == 4) {
...
// Cargar datos
function tabla_principal_cargar_area_p(){
  
  $.getJSON(`${BASE_URL}/area_persona/tabla_principal`, state, function(res){
...
function renderFilas(rows){
  const $tb = $("#tabla-area-persona tbody").empty();
  ...
  rows.forEach(r => {
    let estado = r.estado_trash == '1'
      ...
      : '<span class="badge badge-new">Inactivo</span>';

    $tb.append(`
      <tr class="fila-banco" data-id="${r.idarea_persona}">
        <td  class="py-1 text-center">
          <div class="btn-group btn-group-sm">
            <button class="btn" onclick="ver_editar_area_p(${r.idarea_persona})">
              <i class="fas fa-pencil-alt color_icon_opt"></i>
            </button>
            <button class="btn" onclick="eliminar_area_p(${r.idarea_persona}, '${r.descripcion}')">
              <i class="fas fa-trash color_icon_opt"></i>
            </button>
          </div>
        </td>
        <td class="py-1 text-center">${String(r.idarea_persona).padStart(3,'0')}</td>
...
function marcarOrden(col, dir){
  $("#tabla-area-personas thead th.sortable").each(function(){ const $th = $(this);  const c = $th.data('sort');  $th.removeClass('asc desc'); if (c === col) $th.addClass(dir);  });
}

// Eventos: click en paginación
$("#paginacion").on("click", "a.page-link", function(e){  
  $("#tabla-area-personas tbody").html('<tr><td colspan="15" class="text-center text-muted"><i class="fas fa-sync fa-spin"></i> Buscando...</td></tr>');
  e.preventDefault();   const page = parseInt($(this).data("page"), 10); if (!isNaN(page)){ state.page = Math.max(1, page); tabla_principal_cargar_area_p(); } 
});

// Eventos: ordenar al hacer clic en header
$("#tabla-area-personas thead").on("click", "th.sortable", function(){
  $("#tabla-area-personas tbody").html('<tr><td colspan="15" class="text-center text-muted"><i class="fas fa-sync fa-spin"></i> Ordenando...</td></tr>');
...
$("#buscar").on("input", function(){
  $("#tabla-area-personas tbody").html('<tr><td colspan="15" class="text-center text-muted"><i class="fas fa-sync fa-spin"></i> Buscando...</td></tr>');
  const val = $(this).val(); clearTimeout(t); t = setTimeout(function(){ state.q = val; state.page = 1; tabla_principal_cargar_area_p(); }, 300);
});

// Cambiar tamaño de página
$("#perPage").on("change", function(){
  $("#tabla-area-personas tbody").html('<tr><td colspan="15" class="text-center text-muted"><i class="fas fa-sync fa-spin"></i> Actualizando...</td></tr>');
...
$(".recargar-tabla-area-persona").on("click", function(){
...
function ver_editar_area_p(idarea_persona){

  limpiar_form_area_p();
  $('#modal-agregar-area_p').modal('show');

  $.getJSON(`${BASE_URL}/area_persona/${idarea_persona}/ver-editar`, function (e) {

    if (e.status === true) {
      $("#idarea_persona").val(e.data.idarea_persona);
...
function guardar_y_editar_area_p(e){

  let formData = new FormData($("#form-agregar-area_p")[0]);
  let id = $("#idarea_persona").val();
  let url = '';

  if (id === '') {
    url = `${BASE_URL}/area_persona/crear_area_persona`;
  } else {
    url = `${BASE_URL}/area_persona/editar_area_persona/${id}`;
  ...
  $.ajax({
    ...
    success: function (e) {
      if (e.status === true) {
        ...
        $("#modal-agregar-area_p").modal("hide");
      } else {
        ver_errores(e);
      }
    },
...
function eliminar_area_p(idarea_persona, descripcion){
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      $.ajax({
        url: `${BASE_URL}/area_persona/eliminar_area_persona/${idarea_persona}`,
        type: "PUT",
