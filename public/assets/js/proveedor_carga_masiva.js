$(function () {
  ...
  function mostrarPaso(paso) {
    ...
    // Footer
    if (paso === 2) {
      $('#div-btn-importar-proveedor').removeClass('d-none');
    } else {
      $('#div-btn-importar-proveedor').addClass('d-none');
    }
...
$('#guardar_registro_importar_proveedor').on('click', function () {
  ...
  formData.append('file_excel_proveedor_masivo', file.file);
  formData.append('_token', CSRF);

  $.ajax({
    url: `${BASE_URL}/proveedor/importar_proveedores_excel`,
    type: 'POST',
    data: formData,
    contentType: false,
    processData: false,
    success: function (res) {
      if (res.status) {
        ...
        $('#modal-agregar-proveedor-masivo').modal('hide');
    ...
    xhr: function () {
      ...
      xhr.upload.addEventListener("progress", function (evt) {
        if (evt.lengthComputable) {
          ...
          $("#barra_progress_importar_proveedor").css({"width": percentComplete+'%'}); $("#barra_progress_importar_proveedor").text(percentComplete.toFixed(2)+" %");
        }
      }, false);
      return xhr;
    },
    beforeSend: function () {
      $("#guardar_registro_importar_proveedor").html('<i class="fas fa-spinner fa-pulse fa-lg"></i>').addClass('disabled');
      $("#barra_progress_importar_proveedor").css({ width: "0%",  }).text("0%");
      $("#barra_progress_importar_proveedor_div").show();
    },
    complete: function () {
      $("#barra_progress_importar_proveedor").css({ width: "0%", }).text("0%");
      $("#barra_progress_importar_proveedor_div").hide();
    },    
    error: function (xhr) {
